...
这样找到的往往是文件夹，所以接下来就是处理一个文件目录作为 Node 模块的情况。如果文件目录下有 `package.json`，就根据它的 `main` 字段找到 js 文件。如果没有 `package.json`，那就默认取文件夹下的 `index.js`，如果还没有，就匹配 `index.json`，`index.node`...以此类推。
